...
async function handleDirectory(directory){
  let entryReader = directory.createReader();

  const result = await new Promise((resolve,reject)=>{
    entryReader.readEntries(resolve, reject)
  })
  ...
  for (let i = 0,len = items.length; i < len; i++) {
    ...
    if(entry.isFile){
     console.log('file',entry);
      ...
      entries.length = 0;
      const res = await handleDirectory(entry);
      const files = await fileEntryList2FileList(res);
      console.log('res：',res);
      console.log('files：',files);
      // console.log('directory',entry.createReader());
    }
